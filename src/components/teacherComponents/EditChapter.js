...
import { uploadFile ,deleteFile} from "../../services/teacher.service";
...
const handlePdfFileChange=(e)=>{
  ...
  if(selectedFile){
    ...
    if(selectedFile&&fileType.includes(selectedFile.type)){
      if((
        e.target.files[0].name.toLowerCase().replace(/ /g, '_')!==chapterState.chapterFile.name 
      ||
      e.target.files[0].name.toLowerCase().replace(/ /g, '_')!==props.chapter.chapterFile.name 

        )
        && chapterState.chapterFile)
    { setUpdateFile(true);
...
//submit form------------------------
 ...
 if(updateFile){
  deleteFile({
    path:props.chapter.chapterFile.path
...

      {
        ...
        <input
            ...
            <input type="button" className="action-button-quiz" onClick={addMoreQuestionField} style={{marginRight:"4%"}} value="Add question"/>
              <input type="button" className="action-button-quiz" onClick={submitHandler} value="Update chapter"/>
